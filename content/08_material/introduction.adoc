...
However, existing formats for materials in _3D models_ lack a standardized way to represent and manage multi-spectral materials, which can lead to inconsistencies in simulation scenarios and prevent exchange between simulation tools.
...
If multiple look-up tables of the same kind are linked, for example, BRDF or optical look-up tables, the contained property combinations, for example, wavelength, humidity, and temperature, shall be complementary between the files and shall not overlap.
...
- Accurate simulation of sensor perception:
  To simulate sensors in virtual environments, the _material properties_ of objects shall be accurately defined.
...
- _Material_ reusability across platforms:
  To ensure consistency in simulations, _3D assets_, including their _material_ data, shall be reusable across different tools and platforms.
