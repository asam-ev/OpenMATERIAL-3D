...
Each row in this table maps a _material_ name, given in the 3D data file, or an RGB value, that can be read from a dedicated assignment texture, to a _material property_ file (.xomp).
The RGB code shall be encoded as `"rgb:<R>;<G>;<B>"`, where `<R>`, `<G>`, and `<B>` denote the red, green, and blue channel values as integers in the range 0 to 255.
