...
If the index of a traffic light (`<signal_index>`) is aligned with an ASAM OpenDRIVE map, lighting of individual bulbs can be implemented by corresponding child nodes implementing the {THIS_STANDARD} https://asam-ev.github.io/OpenMATERIAL-3D/asamopenmaterial/latest/specification/07_geometry/general.html#_lighting[lighting concept].
