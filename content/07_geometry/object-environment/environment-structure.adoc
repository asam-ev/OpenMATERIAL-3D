...
If the index of a traffic light (`<signal_index>`) is aligned with an ASAM OpenDRIVE map, lighting of individual bulbs can be implemented by corresponding child nodes implementing the {THIS_STANDARD} xref:../../07_geometry/general.adoc#_lighting[lighting concept].
