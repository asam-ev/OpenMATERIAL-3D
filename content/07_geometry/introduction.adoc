...
This includes metadata, _material_ mappings, and semantic information, such as coordinate system and pivot point descriptions.
...
To accurately replicate how sensors interact with objects in the real world, it is essential to model the behavior of sensors such as cameras, radars, and lidars based on the _material_ and geometric properties of objects in a virtual environment. For this purpose, assets that conform to {THIS_STANDARD} may be necessary in various scenarios, such as:
...
* *Reusing _3D assets_*: Using the same asset file in different simulation tools is only possible if the structure of the assets is standardized.
