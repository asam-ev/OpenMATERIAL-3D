...
The lighting concept in ASAM OpenMATERIAL 3D introduces a clear distinction between spot lights and emissive surfaces, enabling both physically simulated light sources and visually emissive elements to be defined in a standardized way.

=== Spot Lights

Spot lights represent active light sources that emit light into the environment.
They are defined within the XOMA file format using the lightDefinitions property.

Each spot light is associated with a corresponding Grp node in the 3D model file.
This node determines the spatial placement and orientation of the light within the asset.

If the associated Grp node follows a predefined naming convention for lighting group nodes of a specific asset class, the lighting function is implicitly defined.
For example:
A node named Grp_Light_Low_Beam_Left_0 in a vehicle asset indicates that the light represents the left low beam.
In a simulation, this light is activated when the corresponding vehicle function (low beam) is enabled.

If a generic node name is used (e.g., Grp_Light_0), no standardized function can be derived.
In such cases, the simulator may assign a lighting function arbitrarily, resulting in non-standardized behavior.

Additional properties of spot lights, such as intensity, color and beam shape, can be configured in the XOMA file as defined in the corresponding schema.

=== Emissive Surfaces

Emissive surfaces represent materials that visually emit light but do not act as physical light sources in the simulation.
They are defined within the XOMA file format using the emissiveLightMapping property.

Similar to spot lights, emissive surfaces are associated with Grp nodes.
...
For example:
A material associated with the node Grp_Light_Indicator_Left_0 corresponds to the left turn indicator.
In a simulation, the emissive effect is activated when the left indicator is triggered.

As with spot lights, generic node names (e.g., Grp_Light_0) lead to undefined behavior, allowing the simulator to decide the mapping.

The visual appearance of emissive surfaces is defined by materials within the 3D model file.
These materials are referenced in the XOMA file. Additionally:
An emissive texture can be specified in the XOMA file.
