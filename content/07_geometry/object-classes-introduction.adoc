...
If a human is clearly distinguishable from a vehicle, the human and the vehicle are
represented by two separate objects.
Examples are bicycles, motorbikes, or scooters, where the human and the vehicle are
perceived as separate entities.
Stationary objects, such as the road network, traffic infrastructure, vegetation, and buildings, are part
